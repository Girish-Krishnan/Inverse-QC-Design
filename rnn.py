#!/usr/bin/env python3
"""
train_rnn_inverse.py

Train or test an RNN-based inverse quantum circuit synthesis model on the stratified dataset.

Usage (training):
  python train_rnn_inverse.py \
      --mode train \
...
Usage (testing):
  python train_rnn_inverse.py \
      ...
      --model_path rnn_results/checkpoints/best_model.pt \
      --target_index 10 \
      --num_matches 5 \
      --output_vis test_vis.png \
      --device cuda
"""
...
# ------------------------------------------------------------
# RNN-based Generator Model
# ------------------------------------------------------------
class InverseGenModel(nn.Module):
    """
    RNN (GRU) decoder conditioned on p_target + num_qubits. Predicts token sequences.
    """
    def __init__(self, input_dim: int, vocab_size: int, hidden_dim: int = 128):
        super().__init__()
        # Encoder: map [p_target (64) ; n_qubits_norm (1)] -> hidden_dim context
        self.encoder = nn.Sequential(
            nn.Linear(input_dim + 1, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim),
        )
        # Embedding + GRU decoder
        self.embed = nn.Embedding(vocab_size, hidden_dim)
        self.gru   = nn.GRU(hidden_dim, hidden_dim, batch_first=True)
        self.out_fc = nn.Linear(hidden_dim, vocab_size)

    def forward(self, p_target, num_qubits, tgt_ids):
        """
        p_target: (B, 64)
        num_qubits: (B,) int64
        tgt_ids: (B, T) token IDs (including START, excluding final END)
        Returns logits: (B, T, vocab_size)
        """
        B, T = tgt_ids.size()
        nq_norm = (num_qubits.float() / 6.0).unsqueeze(1)      # (B,1)
        enc_in = torch.cat([p_target, nq_norm], dim=1)         # (B,65)
        ctx = self.encoder(enc_in)                             # (B, hidden_dim)
        # Expand context to initialize GRU hidden state
        h0 = ctx.unsqueeze(0)                                   # (1, B, hidden_dim)

        emb = self.embed(tgt_ids)                               # (B, T, hidden_dim)
        output, _ = self.gru(emb, h0)                           # (B, T, hidden_dim)
        logits = self.out_fc(output)                            # (B, T, vocab_size)
        return logits

    def generate_greedy(self, p_target, num_qubits, sos_id, eos_id, max_len=32, device="cpu"):
        """
        Greedy decoding:
        - Start with [SOS]
        - At each step, feed previous tokens to decoder, pick argmax
        - Stop at EOS or max_len
        Returns list of token IDs (including SOS, excluding final EOS)
        """
        seq = [sos_id]
        for _ in range(max_len - 1):
            inp = torch.tensor([seq], dtype=torch.long, device=device)   # (1, t)
            logits = self.forward(p_target.unsqueeze(0), num_qubits.unsqueeze(0), inp)  # (1, t, V)
    ...
    def generate_sample(self, p_target, num_qubits, sos_id, eos_id, max_len=32, device="cpu"):
        """
        Stochastic sampling decoding:
        - Start with [SOS]
        - At each step, feed previous tokens to decoder, sample from softmax distribution
        - Stop at EOS or max_len
        Returns list of token IDs (including SOS, excluding final EOS)
        """
        seq = [sos_id]
        for _ in range(max_len - 1):
            inp = torch.tensor([seq], dtype=torch.long, device=device)   # (1, t)
            logits = self.forward(p_target.unsqueeze(0), num_qubits.unsqueeze(0), inp)  # (1, t, V)
            probs = torch.softmax(logits[0, -1], dim=-1).cpu().detach().numpy()       # (V,)
...
def entanglement_entropy_loss(true_probs, pred_probs, num_qubits):
    ...
    Uses Qiskit to reconstruct a statevector from probs and compute single-qubit entropy.
    """
    try:
        n = num_qubits.item()
...
def train_loop(args):
    ...
    # Instantiate model
    model = InverseGenModel(input_dim=64, vocab_size=len(tokenizer), hidden_dim=args.hidden_dim)
...
def test_loop(args):
    ...
    while len(matches) < args.num_matches and attempts < args.max_attempts:
        ...
        # Since dictionary only stores nonzero probs, zero probs could be missing:
        is_match = True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train or test RNN-based inverse QC model")
    ...
    train_parser.add_argument("--hidden_dim", type=int, default=128, help="RNN hidden dimension")
    ...
    train_parser.add_argument("--output_dir", type=str, default="results/rnn",
    ...
    test_parser.add_argument("--output_vis", type=str, default="test_vis.png",
